...
int StartGameLoop(void* hInst, int nCmdShow) {
    ...
    printf("[G72 LOG] iniciado correctamente.\n");
    ...
    printf("[G72 LOG] configuracion de pantalla a %dx%d...\n", SCREEN_WIDTH, SCREEN_HEIGHT);
    ...
    if (ChangeDisplaySettings(&dmScreenSettings, CDS_FULLSCREEN) != DISP_CHANGE_SUCCESSFUL) {
        printf("[G72 WARNING] Usando ventana por defecto.\n");
    }
    ...
    printf("[G72 LOG] Entrando al bucle principal\n");
    ...
    while (running) {
        ...
        switch (current_state) {
            case STATE_TITLE:
                ...
                if (keys[VK_RETURN]) { 
                    printf("[G72 LOG] Deteniendo audio de 'BIOS'...\n");
            ...
            case STATE_LOADING_STAGE:
                ...
                printf("[G72 LOG]  volcado\n");
                init_stage0();
                Sleep(1500); 

                current_state = STATE_TEST_STAGE;
                printf("[G72 LOG] completado.\n");
